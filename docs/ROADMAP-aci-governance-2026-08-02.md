...
- 已落地却被低估的：① **响应体大小处理已完备**（2MB 硬上限 + 15万字符截断 + gzip，与主程序 `browser_read` 切片同源思想）；② **超时已具备**（控制器 15s 硬上限 + OkHttp 10/14/14s）；③ **多受控端统一控制台入口已上线**（`QuroAidlAciCenterScreen` 已列全部 App + 每 App「打开控制台」）。
...
| 关键行动项 | 7 条（见行动清单） |
| 建议负责人 | 主程序 ACI 团队（QuroMainAciService / QuroAidlAciManager / QuroPrivilegeAudit 维护者） |
...
| 3 | 请求审计（URL/方法/状态码/耗时持久化） | 🔴 控制端缺失 | `QuroAidlAciManager.kt:279,308,312`：`call()` 仅 `Log.d/w`（Logcat 易失）；`QuroPrivilegeAudit.kt` 仅记录权限提升，不记录 ACI/HTTP 调用。浏览器侧有 `browser_audit`（受控端自记） | P0 |
| 4 | 控制台操作日志持久化 / 事后回放 | 🔴 控制端缺失 | `QuroAidlAciCenterScreen.kt:375-409`：`openConsole`/`consoleAction` 不落盘；无回放 | P1 |
...
| 7 | 超时/重试/熔断 | 🟡 超时已有、重试/熔断无 | `QuroMainAciService.kt:107-133`：`HARD_TIMEOUT_S=15` 硬上限 + OkHttp connect 10s/read 14s/write 14s；`QuroAidlAciManager.kt:329` 仅对 `RemoteException` 重绑重试，无业务级重试/熔断 | P0（补重试/熔断） |
...
4. **控制端 ACI 调用审计**（最大真实治理缺口，落点 新建 `QuroAidlAciCallAudit.kt` + 扩展 `QuroAuditScreen`）
...
1. **多受控端统一控制台入口**：⚠️ **已上线**，用户低估。`QuroAidlAciCenterScreen.kt:547-573` 已列出所有发现的 ACI App，每张卡 `onOpenConsole` 拉取该 App 的 `console_ui` 渲染。下一步是体验增强（记住每 App 上次快照、免重开 Dialog 切换），而非从零建设。
2. **控制台内可观测性视图**：复用 SDUI，新增"编排观测"能力或本地屏读取 `QuroAidlAciCallAudit`，以组件化 JSON 渲染调用链/耗时/成功率/LLM 决策轨迹。
3. **控制台操作回放与导出**：基于 `QuroAidlAciCallAudit` 记录，支持重放某次 `console_action`/`http_request` 并导出 JSON。
...
| # | 行动 | 负责方 | 紧急度 | 期望完成 |
|---|------|--------|--------|---------|
| 1 | 控制端 ACI/HTTP 调用审计：新建 `QuroAidlAciCallAudit`（复用 `QuroPrivilegeAudit` 模式），`QuroAidlAciManager.call` 落盘调用记录；审计页加 Tab | 主程序 ACI 团队 | P0（现在可做，低风险高价值） | 下个补丁 |
...
- 本次评审仅核实**主程序** `QuroMainAciService`/`QuroAidlAciManager`/`QuroAidlAciCenterScreen`/`AciConsoleModel` 与受控仓 `http_request` 文档描述（docs 第 241–253 行）。未在真机跑 `http_request` 四例（GET/POST/大响应/超时）验证，建议与 v1.0.14 真机验证一并闭环。
...
- 源码证据索引：
  ...
  - `app/src/main/java/com/ai/assistance/quro/core/aci/QuroAidlAciManager.kt`（call 仅 Logcat：279/308/312；RemoteException 重绑：329）
  - `app/src/main/java/com/ai/assistance/quro/ui/QuroAidlAciCenterScreen.kt`（多控台入口已上线：547-573）
