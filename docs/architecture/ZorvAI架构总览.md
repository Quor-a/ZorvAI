...
### 构架介绍
Web 应用是 ZorvAI 内「AI 生成即可运行」的可视化 Web 应用，核心是 **MiniAppFramework**：把 LLM 产出的 HTML/JS 在应用内渲染容器中沙箱运行，支持同目录资源（图片自动转 `data URI`）、私有 `app://` 协议、对话框内联预览与独立面板编辑。用户/AI 可在面板新建、编辑、运行 Web 应用，无需打包上架。

- **内联渲染**：HTML/JS 跑在应用内 WebView/内联容器，免安装、即写即跑。
- **资源自包含**：同目录图片预览时转 `data URI`，与面板内 `app://` 渲染一致。
- **安全写入**：工程名/路径安全化，防止 `..` 越界写入 工程目录外。

### 分层技术构架
```mermaid
graph TB
  subgraph L5[集成层]
    A1[对话框内联渲染] --> A2[工具中心 Web 应用面板]
    A2 --> A3[节点编辑器联动 / 分享导出]
  end
  subgraph L4[运行层]
  ...
  subgraph L3[编辑层]
    C1[Web 应用面板 新建/编辑/删除] --> C2[run 对话框预览]
    C2 --> C3[模板示例 / 快照状态]
  end
  subgraph L2[生成层]
...
| 组件 | 职责 | 说明 |
|---|---|---|
| MiniAppFramework | Web 应用运行内核 | 内联渲染 HTML/JS、沙箱隔离 |
| Web 应用面板 | 可视化编辑入口 | 新建示例、编辑、删除、运行 |
| run 对话框预览 | 对话框内联运行 | 同目录图片转 data URI，与 app:// 一致 |
| app:// 协议 | 私有资源寻址 | 面板内资源定位，与预览同源 |
| writeFlow | 工程写入 | 工程名/路径防 `..` 越界 |

### 关键流程
用户/AI「新建示例」→ AI 生成 HTML/JS → 安全化校验 → `writeFlow` 落盘工程目录（filesDir/miniapp） → MiniAppFramework 内联渲染（图片转 data URI）→ 面板/对话框运行 → 导出/分享。
...
graph LR
  ...
  Terminal -->|本地执行| MiniApp[Web 应用]
  MCP -->|外部能力| Browser
  Skills -->|复用| MiniApp
  Voice -->|口播| Digital[数字人控制台]
```

> 浏览器 2.0 是「行动入口」，终端 4.0 是「执行底座」，技能/Skills 与 MCP 是「能力扩展面」，语音服务是「输入/输出总线」， Web 应用是「轻量可编程面」，数字人控制台是「拟人呈现终端」。
