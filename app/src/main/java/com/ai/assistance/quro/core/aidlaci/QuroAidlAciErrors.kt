 ...
 *
 * 本对象是主应用 ACI 层（控制端 QuroAidlAciManager + 受控端 QuroMainAciService）
 * 的一部分，直接消费 aci-core 的公开 API：用 AidlAciResponse.error(code, json)
 * 把结构化错误塞进 errorMessage，用 getErrorCode()/getErrorMessage() 解析。
 * 不改动 aci-core 内核（其源码不在本仓，仅以 AAR 形式依赖），而是在其上做
 * 语义化错误层 —— 与 aci-core 的 AidlAciError 常量共存，错误码落在独立的
...
object QuroAidlAciErrors {
    ...
    /**
     * 从 aci-core 的 errorMessage 中解析结构化错误（受控端经 AidlAciResponse.error(code, toJson()) 写入）。
