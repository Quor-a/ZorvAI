 ...
 * for Compose.
 *
 * 移植自 Kai (com.inspiredandroid.kai) 的自研 VT100/xterm 引擎，去品牌化、去
...
class TerminalScreen(
    ...
    /**
     * OSC 8 / stream / open-url URIs. Keyed by scheme+host+path so OAuth authorize
     * variants (different redirect_uri) collapse to one entry; value is the best
     * (usually longest) full URL for that path.
     */
    ...
    fun snapshot(): TerminalSnapshot {
        ...
        return TerminalSnapshot(
            ...
            hyperlinks = hyperlinks.values.toList(),
        )
    }
    ...
    private companion object {
        /** Enough for one auth URL + a docs link; keeps setup from flooding. */
