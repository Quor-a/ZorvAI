...
/**
 * 应用内 Linux 环境工具集（proot + Alpine aarch64）。
 *
 * v108 删除了 QuroToolsLinux.kt，此文件为 v116 恢复。后端 [QuroLinuxEnv] 在
 * proot 二进制与 Alpine rootfs 资产齐备时真实执行命令；资产缺失时优雅降级并报明确原因。
 *
...
class LinuxRunTool : QuroTool {
    ...
    override val description: String =
        "在应用内 proot + Alpine Linux 环境中执行一条 shell 命令并返回输出（如 uname -a / apk --version / python3 -c）。需 Linux 环境资产就绪。"
    ...
    override fun run(context: Context, arguments: String): String {
        ...
        if (!QuroLinuxEnv.probe(context).available) {
            ...
            return "⏳ Linux 环境未安装，已自动在后台开始安装（下载 Alpine rootfs 并初始化），请稍候在终端查看进度后重试。"
        }
...
class LinuxInstallTool : QuroTool {
    ...
    override val description: String =
        "在应用内 Linux 环境用 apk 安装一个 Alpine 软件包（如 python3 / nodejs / git）。"
    override val parametersJson: String =
    ...
    override fun run(context: Context, arguments: String): String {
        ...
        val (code, out) = QuroLinuxEnv.run(context, "apk add --no-cache $pkg")
...
class LinuxStartTool : QuroTool {
    ...
    override val description: String =
        "检查并初始化应用内 Linux 环境，返回就绪状态（proot 二进制与 Alpine rootfs 是否就位）。"
...
class LinuxStatusTool : QuroTool {
    ...
    override val description: String =
        "返回应用内 Linux 环境的探测状态：proot 二进制路径、Alpine rootfs 路径、是否可用。"
    override val parametersJson: String = """{"type":"object","properties":{}}"""
