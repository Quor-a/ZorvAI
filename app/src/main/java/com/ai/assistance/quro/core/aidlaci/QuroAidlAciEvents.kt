    ...
    fun emit(type: String, pkg: String, detail: String) {
        ...
        synchronized(lock) { listeners.toList() }.forEach { listener ->
            runCatching { listener(e) }.onFailure { t ->
                android.util.Log.w("QuroAidlAciEvents", "订阅者处理事件异常：${t.message}")
            }
        }
    }
}
