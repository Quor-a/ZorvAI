...
"""
    ...
    private fun installQuroTermCompat(context: Context, rootfs: File) {
        try {
            val marker = File(rootfs, "usr/local/bin/.quro_term_compat_installed")
            if (marker.exists()) return

            val targetScript = File(rootfs, "usr/local/bin/quro_term_compat.sh")
            try {
                context.assets.open("linux_env/quro_term_compat.sh").use { input ->
                    FileOutputStream(targetScript).use { output ->
                        input.copyTo(output)
                    }
                }
                targetScript.setExecutable(true, false)
                marker.writeText("installed")
                Log.i(TAG, "✅ QuroTerm 双兼容层已安装")
            } catch (e: Exception) {
                Log.w(TAG, "⚠ QuroTerm 双兼容层安装失败: ${e.message}")
            }
        } catch (e: Exception) {
