...
const val AIDL_ACI_SDUI_SCHEMA_VERSION = "aci-sdui-v1"

data class AidlAciScreen(
    val title: String,
    val subtitle: String,
    val updatedAt: Long,
    val components: List<AciComponent>,
    /** SDUI 快照 schema 版本；受控端未下发时为空串（按 [AIDL_ACI_SDUI_SCHEMA_VERSION] 兼容处理）。 */
...
/** 将受控端 console_ui 下发的 JSON 解析为 [AidlAciScreen]。 */
object AciConsoleModel {
    fun parse(json: JSONObject): AidlAciScreen {
        ...
        if (schemaVersion.isNotEmpty() && schemaVersion != AIDL_ACI_SDUI_SCHEMA_VERSION) {
            // 版本不一致：向后兼容——仍解析，仅告警（未来可在此做字段映射/降级）
            Log.w("AciConsoleModel", "⚠️ SDUI schema 版本不一致：收到=$schemaVersion，本端=$AIDL_ACI_SDUI_SCHEMA_VERSION，按兼容模式解析")
        }
        return AidlAciScreen(
