 ...
 *
 * 旧版控制端 [QuroAciManager] 直接持有 IACIService（Binder）并内联调用逻辑；
 * 抽象出 [AciAdapter] 后，未来可无缝接入 WebSocket / HTTP / gRPC 等传输，
 * 第三方只需实现一套 Adapter 即可被 ZorvAI 调度，无需关心底层传输与鉴权。
 *
 * 当前落地：[BinderAciAdapter] 包裹既有 Binder 调用（委托给 [QuroAciManager] 的
 * 超时/重试/审计逻辑），不改热路径行为；后续 Phase 再派生 WsAciAdapter / HttpAciAdapter。
 */
interface AciAdapter {
    ...
    /** 同步调用某能力。 */
    fun call(capability: String, params: Bundle): ACIResponse
...
/**
 * Binder 传输的 Adapter 实现：复用既有 [QuroAciManager] 调用链路（超时/重试/审计/协议协商）。
 * 通过构造函数注入 call / caps / protocol 三个 provider，避免与 [QuroAciManager] 形成循环依赖，
 * 也让 Adapter 成为纯粹的「传输适配」而非逻辑副本。
 */
class BinderAciAdapter(
    private val pkg: String,
    @Suppress("UNUSED_PARAMETER") private val service: IACIService,
    private val callFunc: (String, Bundle) -> ACIResponse,
    ...
    override fun call(capability: String, params: Bundle): ACIResponse = callFunc(capability, params)
