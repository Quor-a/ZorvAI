...
object QuroToolUsageHints {
    ...
    val TOOL_USAGE_HINTS: Map<String, String> = mapOf(
        ...
        "open_web" to "「打开百度」「用浏览器看这个网页」「访问 XX 网址」都调用",
        "ai_browser" to "多用途：既能「后台联网搜索资料」也能「打开网页自动填表/点按钮/抓取正文」也能「下载网页里的文件」——需要 AI 自动上网办事时调用",
        ...
        "ui_widget" to "多用途：既能「展示一个开关让用户点」也能「画一张图表/进度条/表格/评分」也能「放个倒计时或表单」——需要对话框里出现可交互 UI 时调用",
    ...
    /**
     * 强指令：必须真正调用工具执行，而非仅描述；把用户任意措辞/换说法/换场景的意图映射到最贴近的工具。
     * 注入到系统提示词「我的能力」段开头，作为铁律。
     */
    fun buildToolUseDirective(): String = """
# 工具调用铁律（必须严格遵守）
1. 只要用户的话涉及「做一件事」（打开/查/发/写/读/播/控制/提醒/搜/生成/执行…），你必须调用对应的工具函数**真正去执行**，绝不能用自然语言「描述」你要做什么，也绝不能假装已完成。
2. 用户措辞千变万化：同一功能会有无数种说法与场景。你要按「意图」而非「关键词」匹配工具——例如「帮我把下午三点的会记一下」「加个日程」「提醒我周五开会」都对应 write_calendar；「帮我按这个地址走」「这地方在哪儿」对应 geocode/get_location。
3. 一个工具往往有多种用途：execute_intent 既能「打开某 App 深层页面」也能「启动 Service/发隐式 Intent」；send_broadcast 既能「发系统广播」也能「通知其他组件」；ui_widget 既能「展示开关」也能「画图表/进度/表格」；ai_browser 既能「联网搜索」也能「自动填表/抓正文/下载」。按用户真实意图选用，不要被单一用例限制。
4. 不确定用哪个工具时，选「最贴近用户意图」的那个并调用；不要因为措辞不标准就放弃调用或只回文字。宁可调用近似工具，也不要纯文字应答。
5. 多个独立动作可一次性并行发起（一次返回多个 tool_calls）；拿到工具结果后，如需进一步动作继续调用，直到任务完成。
6. 下方工具清单（名称 + 多用途说明）与你可调用的函数严格一致：只调用清单里出现的工具名，不要编造清单外的工具。
""".trimIndent()
}
