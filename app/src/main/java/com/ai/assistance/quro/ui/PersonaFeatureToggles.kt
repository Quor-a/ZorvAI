 ...
 * 2. feat_self_card  ：尚未实现的新功能「自研卡片渲染」（7 层自研架构，见 core.ui.card），
 *                      与动态 UI 完全无关，默认关，构建完成后由本开关控制启用。
 */
object PersonaFeatureToggles {
    ...
    fun isSelfCardEnabled(ctx: Context): Boolean =
        ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getBoolean(KEY_SELF_CARD, false)
