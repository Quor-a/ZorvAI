...
function(quro_normalize_source_token out_var token)
...
function(quro_git_ref_var out_var dependency_name git_ref)
    string(TOUPPER "${dependency_name}" dependency_key)
    string(REPLACE "-" "_" dependency_key "${dependency_key}")

    set(ref_var "QURO_${dependency_key}_GIT_REF")
...
function(quro_resolve_git_ref out_var repository git_ref)
...
function(quro_github_archive_url out_var repository resolved_sha)
    ...
    if("${archive_url}" STREQUAL "${repository_without_suffix}")
        message(FATAL_ERROR "Only GitHub archive sources are supported by quro_github_archive_url: ${repository}")
    endif()

    set(${out_var} "${archive_url}" PARENT_SCOPE)
endfunction()

function(quro_declare_git_source dependency_name repository git_ref)
    quro_git_ref_var(ref_var "${dependency_name}" "${git_ref}")
    quro_resolve_git_ref(resolved_sha "${repository}" "${${ref_var}}")
    quro_github_archive_url(archive_url "${repository}" "${resolved_sha}")
    quro_normalize_source_token(source_token "${dependency_name}-${resolved_sha}")

    set(deps_root "${CMAKE_SOURCE_DIR}/.cxx/quro_deps")
...
function(quro_populate_git_source out_source_dir out_binary_dir dependency_name)
...
function(quro_prepare_git_source out_source_dir out_binary_dir dependency_name repository git_ref)
    quro_declare_git_source("${dependency_name}" "${repository}" "${git_ref}" ${ARGN})
    quro_populate_git_source(${out_source_dir} ${out_binary_dir} "${dependency_name}")
