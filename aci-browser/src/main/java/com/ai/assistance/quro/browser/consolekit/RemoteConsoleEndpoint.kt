 ...
 * @param serviceClassName 目标 App 的 ACI Service 完整类名（默认 `aci.QuroControlledAciService`）。
 */
class RemoteConsoleEndpoint(
    private val context: Context,
    private val targetPackage: String,
    private val serviceClassName: String = "aci.QuroControlledAciService"
) : AciConsoleContract {

    @Volatile private var binder: IACIService? = null
    private val conn = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, b: IBinder?) {
            binder = IACIService.Stub.asInterface(b)
        }
    ...
    override fun sendAction(action: String, payload: Map<String, String>): JSONObject {
        ...
        val req = ACIRequest("console_action", b)
        runCatching { req.setCallerPkg(context.packageName) }
        val resp = runCatching { svc.call(req) }.getOrNull()
